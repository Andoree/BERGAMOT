...
from graphmel.models.gatv2_dgi_sapbert import GATv2DGISapMetricLearning
...
from graphmel.data.dataset import load_positive_pairs, map_terms2term_id, create_term_id2tokenizer_output, \
    load_data_and_bert_model, convert_edges_tuples_to_oriented_edge_index_with_relations
...
def parse_args():
    ...
    parser.add_argument('--gat_num_outer_layers', type=int,
                        help="Maximum order of neighbors (K-hop neighbors). In our experiments, it is usually equal "
                             "to 1 due to computational cost.")
    parser.add_argument('--gat_num_inner_layers', type=int,
                        help="Number of GAT layers for K-hop neighbors given a fixed K")
    parser.add_argument('--gat_num_hidden_channels', type=int,
                        help="Hidden size of intermediate (not input and output layers as their dimensionality is equal "
                             "to LM's dimensionality) GAT layers")
    parser.add_argument('--gat_num_neighbors', type=int, nargs='+',
                        help="List of neighborhood sizes for 1-, 2-, ... K-hop neighbors. We only pass neighborhood "
                             "size for 1-hop neighbors")
    ...
    parser.add_argument('--remove_selfloops', action="store_true",
                        help="Whether self-loops in graph should be removed. We remove self-loops to enforce passing"
                             "information from neighbors rather than a central node itself.")
    ...
    parser.add_argument('--layernorm_output', action="store_true",
                        help="Whether to apply LayerNorm to output of graph encoder")
    ...
    parser.add_argument('--loss', choices=("ms_loss",), default="ms_loss")
...
def main(args):
    ...
    output_subdir = f"gatv2_{'.'.join((str(x) for x in args.gat_num_neighbors))}_{args.gat_num_hidden_channels}" \
                    ...
                    f"_lr_{args.learning_rate}_blr_{args.bert_learning_rate}_b_{args.batch_size}" \
                    f"_lnorm_{args.layernorm_output}"
    ...
    model = GATv2DGISapMetricLearning(bert_encoder, gat_num_hidden_channels=args.gat_num_hidden_channels,
                                      ...
                                      layernorm_output=args.layernorm_output).to(device)

    start = time.time()
    train_graph_sapbert_model(model=model, tokenizer=bert_tokenizer, train_epoch_fn=train_gatv2_dgi_sapbert,
